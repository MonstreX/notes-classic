...
export const mountTextImportModal = (
  root: HTMLElement,
  handlers?: { onImportStart?: () => void; onImportEnd?: () => void }
): TextImportModal => {
  let isOpen = false;
  let summary: Awaited<ReturnType<typeof scanTextSource>> | null = null;
  let reportPath = "";
  ...
  overlay.innerHTML = `
    <div class="import-modal__panel">
      <div class="import-modal__header">
        <h3 class="import-modal__title">${t("import_text.title")}</h3>
      ...
      <div class="import-modal__body">
        ...
        <div class="import-modal__path" data-import-path>${t("import_text.path_empty")}</div>
      ...
      <div class="import-modal__footer">
        <button class="import-modal__action" data-import-select type="button">${t("import_text.select_folder")}</button>
        <button class="import-modal__action import-modal__action--primary" data-import-run type="button" disabled>${t("import_text.import")}</button>
  ...
  const setSummary = (nextSummary: typeof summary) => {
    ...
    summaryEl.innerHTML = `
      <div class="import-summary__row"><span>${t("import_text.summary.notes")}</span><span>${summary.noteCount}</span></div>
      <div class="import-summary__row"><span>${t("import_text.summary.notebooks")}</span><span>${summary.notebookCount}</span></div>
      <div class="import-summary__row"><span>${t("import_text.summary.stacks")}</span><span>${summary.stackCount}</span></div>
      <div class="import-summary__row"><span>${t("import_text.summary.attachments")}</span><span>${summary.attachmentCount}</span></div>
      <div class="import-summary__row"><span>${t("import_text.summary.images")}</span><span>${summary.imageCount}</span></div>
    `;
  };
  ...
  const handleSelect = async () => {
    setStatus("", "muted");
    setReport("");
    const selected = await invoke<string | null>("select_text_folder");
    if (!selected || !pathEl) return;
    pathEl.textContent = selected;
    setStatus(t("import_text.scanning"), "muted", true);
    try {
      summary = await scanTextSource(selected);
      if (!summary.valid) {
        setStatus(t("import_text.scan_failed_generic"), "error");
        return;
      }
      setSummary(summary);
      setStatus(t("import_text.ready"), "ok");
      if (runBtn) runBtn.disabled = false;
    } catch (e) {
      logError("[import] text scan failed", e);
      setStatus(t("import_text.scan_failed", { message: String(e) }), "error");
    }
  };
  ...
  const handleRun = async () => {
    if (!pathEl) return;
    if (!summary) {
      const selected = (pathEl.textContent || "").trim();
      if (!selected || selected === t("import_text.path_empty")) {
        setStatus(t("import_text.path_empty"), "error");
        return;
      }
      setStatus(t("import_text.scanning"), "muted", true);
      try {
    ...
    try {
      ...
      setStatus(t("import_text.finished"), "ok");
