...
export const mountObsidianImportModal = (
  root: HTMLElement,
  handlers?: { onImportStart?: () => void; onImportEnd?: () => void }
): ObsidianImportModal => {
  let isOpen = false;
  let summary: Awaited<ReturnType<typeof scanObsidianSource>> | null = null;
  let reportPath = "";
  ...
  overlay.innerHTML = `
    <div class="import-modal__panel">
      <div class="import-modal__header">
        <h3 class="import-modal__title">${t("import_obsidian.title")}</h3>
      ...
      <div class="import-modal__body">
        <div class="import-modal__hint">
          ${t("import_obsidian.hint")}
        </div>
        <div class="import-modal__path" data-import-path>${t("import_obsidian.path_empty")}</div>
      ...
      <div class="import-modal__footer">
        <button class="import-modal__action" data-import-select type="button">${t("import_obsidian.select_folder")}</button>
        <button class="import-modal__action import-modal__action--primary" data-import-run type="button" disabled>${t("import_obsidian.import")}</button>
  ...
  const setSummary = (nextSummary: typeof summary) => {
    ...
    summaryEl.innerHTML = `
      <div class="import-summary__row"><span>${t("import_obsidian.summary.notes")}</span><span>${summary.noteCount}</span></div>
      <div class="import-summary__row"><span>${t("import_obsidian.summary.notebooks")}</span><span>${summary.notebookCount}</span></div>
      <div class="import-summary__row"><span>${t("import_obsidian.summary.stacks")}</span><span>${summary.stackCount}</span></div>
      <div class="import-summary__row"><span>${t("import_obsidian.summary.attachments")}</span><span>${summary.attachmentCount}</span></div>
      <div class="import-summary__row"><span>${t("import_obsidian.summary.images")}</span><span>${summary.imageCount}</span></div>
    `;
  };
  ...
  const setStageProgress = (
    ...
    if (!stage) return;
    if (state === "running") {
      const title = t("import_obsidian.running");
  ...
  const reset = () => {
    if (pathEl) pathEl.textContent = t("import_obsidian.path_empty");
    setStatus("", "muted");
  ...
  const handleSelect = async () => {
    setStatus("", "muted");
    setReport("");
    const selected = await invoke<string | null>("select_obsidian_folder");
    if (!selected || !pathEl) return;
    pathEl.textContent = selected;
    setStatus(t("import_obsidian.scanning"), "muted", true);
    try {
      summary = await scanObsidianSource(selected);
      if (!summary.valid) {
        setStatus(t("import_obsidian.scan_failed_generic"), "error");
        return;
      }
      setSummary(summary);
      setStatus(t("import_obsidian.ready"), "ok");
      if (runBtn) runBtn.disabled = false;
    } catch (e) {
      logError("[import] obsidian scan failed", e);
      setStatus(t("import_obsidian.scan_failed", { message: String(e) }), "error");
    }
  };
  ...
  const handleRun = async () => {
    ...
    try {
      const shouldReplace = await confirmReplaceIfNeeded({
        title: t("import_obsidian.replace_title"),
        message: t("import_obsidian.replace_message"),
      ...
      await handleImportResult({
        ...
        setReport,
        texts: {
          finished: t("import_obsidian.finished"),
          finishedErrors: t("import_obsidian.finished_errors"),
          failed: t("import_obsidian.failed"),
          reportSavedKey: "import_obsidian.report_saved",
          ...
          restartMessage: t("import_obsidian.restart"),
        },
      });
    } catch (err) {
